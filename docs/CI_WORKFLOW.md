...
All merge-surface Python validation jobs (`Ruff + Mypy`, `Pytest`, `MkDocs`, and `Docs Build`) install a shared baseline (`requirements.lock` + `requirements-dev.lock`) before job-specific setup (for example `requirements-docs.lock`) and before `check_env.py --auto-install`, so 3.11/3.12 environments resolve the same dependency contract.
